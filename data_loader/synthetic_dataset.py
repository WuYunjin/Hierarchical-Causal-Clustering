...
class SyntheticDataset(object):
    ...
    def _setup(self):
        # self.matrix is a list with size num_groups, each element is a binary tensor with shape (pl+1,m,m)
        self.matrix = SyntheticDataset.simulate_all_dag(self.num_groups, self.num_variables, self.max_lag)

        # self.X is a list with size num_groups, each element is a numpy.array with shape(num_subjects_per_group,Ts,m)
    ...
    def simulate_all_sem(matrix, num_groups, num_subjects_per_group, num_samples, num_variables, noise_distribution='MoG', num_gaussian_component=5):
        ...
        for k in range(num_groups):
            ...
            # if k==0:
            #     mu_k_low, mu_k_high = 0.1, 0.2
            #     nu_k_low, nu_k_high = 0.1, 0.2
            # elif k==1:
            #     mu_k_low, mu_k_high = 0.2, 0.3
            #     nu_k_low, nu_k_high = 0.2, 0.3
            # else:
            #     mu_k_low, mu_k_high = 0.3, 0.4
            #     nu_k_low, nu_k_high = 0.3, 0.4
            ...
            # Each group has the same MoG distribution.
            ...
            for s in range(30): # 30 is the max num_subjects_per_group, to make sure the generated data is consistent when num_subjects_per_group is different.
    ...
    def simulate_sem(max_lag, num_samples, num_variables, B_k, A_k, noise_distribution, Pi_k_prime, Mu_k_prime, Sigma_k_prime):
        ...
        # we normalize  X, in case it become larger and larger when coefficient>1.
        # Referred from: https://blog.csdn.net/kane7csdn/article/details/86475918
